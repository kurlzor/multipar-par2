// search.c
// Copyright : 2015-09-14 Yutaka Sawada
// License : GPL
...
int search_recovery_files(int check_all)
{
	...
	if (check_all & 2){	// 追加検査なら同じディレクトリ内の全ファイル
...
int search_main_packet(
	...
	int switch_v)			// 検査レベル 0=全領域, +1=後半分だけ
{
		...
		for (i = 0; i < file_num; i++){
			...
			if (off != 0){
				if (off == 1){	// ディレクトリ記号を含んでる
					...
					if (off == 16){
						max++;
						printf("file %d: filename is invalid\n", i);
					} else {
						utf16_to_cp(uni_buf, ascii_buf, cp_output);
						printf("file %d: sanitized to \"%s\"\n", i, ascii_buf);
						if (off & 4){	// 文字数が変わった
							k = (int)wcslen(list_buf + files[i].name);
							list_len = remove_file_path(list_buf, list_len, files[i].name);
							for (j = 0; j < file_num; j++){
