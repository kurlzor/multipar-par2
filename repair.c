// repair.c
// Copyright : 2016-06-25 Yutaka Sawada
// License : GPL
	...
	source_ctx_r *s_blk)	// 各ソース・ブロックの情報
{
	...
	for (num = 0; num < entity_num; num++){
		...
		if ((files[num].size > 0) && ((files[num].state & 0x80) == 0) &&
				((files[num].state & 7) != 0)){	// チェックサムと作業ファイルが存在するなら
			// 作業ファイルを開く
...
/*					printf("copy block : off 0x%I64X block %d -> off 0x%I64X block %d\n",
						...
						src_blk, file_off, i);*/
					if ((files[src_file].state & 7) != 0){	// 作り直した作業ファイルから読み込む
						get_temp_name(list_buf + files[src_file].name, file_path + base_len);
					} else if (files[src_file].state & 0x20){	// 名前訂正失敗時には別名ファイルから読み込む
					...
					hFile_src = CreateFile(file_path, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	...
	source_ctx_r *s_blk)	// 各ソース・ブロックの情報
{
	...
	for (num = 0; num < entity_num; num++){
		// 新しく作り直したソース・ファイルなら
		if ((files[num].size > 0) && ((files[num].state & 7) != 0)){
			bad_flag = 0;
			// 再度開きなおす
			wcscpy(file_path + base_len, list_buf + files[num].name);
			...
			if (bad_flag){	// 失敗
				if (((files[num].state & 0x80) == 0) &&
						(bad_flag == 2) && (switch_b & 4)){	// 修復に失敗した場合でも、元のファイルを置き換える
					// 完全なブロックが含まれてるかどうか
					b_last = files[num].b_off + files[num].b_num;
					for (i = files[num].b_off; i < b_last; i++){
						if (s_blk[i].exist != 0){
	...
	file_ctx_r *files)		// 各ソース・ファイルの情報
{
	...
	wcscpy(file_path, base_dir);
	for (num = 0; num < entity_num; num++){
		if ((files[num].size > 0) && ((files[num].state & 7) != 0)){	// 作業ファイルが存在するなら
			// 作業ファイルを削除する
			get_temp_name(list_buf + files[num].name, file_path + base_len);
			//printf("delete %S\n", file_path);
