/*----------------------------------------------------------------------------
;
; MD5 hash generator -- Paul Houle (paulhoule.com) 4/29/2010
;
...
// Prototype external core routine (95%+ of the time is spent there).
extern void Phmd5DoBlocks(unsigned char *hash, char *pdata, unsigned dbytes);
extern void Phmd5DoBlocksZero(unsigned char *hash, unsigned dbytes);
...
void Phmd5Process(PHMD5 *pmd5, char *pdata, unsigned bytecnt) {
	...
	if (bytecnt & ~63 && ((unsigned) pdata & 3) == 0) {
	...
	while (bytecnt) {					// handle residual/non-aligned data
		unsigned cb = 64 > bytecnt ? bytecnt : 64;
...
void Phmd5ProcessZero(PHMD5 *pmd5, unsigned bytecnt) {
	unsigned resid = (unsigned) pmd5->totbyt;
