...
static unsigned long time_f_u(FILETIME *file_time)
{
...
static void base32_encode4(
	...
	unsigned long v3,	// ファイル番号は 3バイトまで
	unsigned long v2,
	unsigned long v1,
	unsigned long v0)
{
...
// 置換文字の番号を返す
static long replace_decode(wchar_t c)
{
	// 置換用の文字は「 "';=[\」を除外して 24個
	wchar_t replace_table[] = L"!#$%&()*,-.:<>?@]^_`{|}~";
	long i;
...
// 伸張する
static void expand_text(wchar_t *buf, long orig_count)
{
	long i, j, len, cur_count;

	cur_count = (long)wcslen(buf);
...
// BASE64 文字の番号を返す
static long base64_decode(wchar_t c)
{
	wchar_t base64_table[] = L"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	long i;
...
static unsigned long base64_decode5(
	wchar_t *buf,			// 最大で 27文字分必要 (末尾の null 文字含めて)
	unsigned long state[4])
{
	unsigned long v0, v1, v2, v3, v4; 
...
long check_ini_file(unsigned char *set_id, wchar_t *recv_path)
{
	unsigned char set_data[HEADER_SIZE * 2];
	wchar_t path[MAX_LEN], ini_name[INI_NAME_LEN + 1];
	long i, match;
	unsigned long file_time, time_now, time_limit;
	HANDLE hFind;
	...
	file_ctx_r *files)	// 各ソース・ファイルの情報
{
	...
	for (i = 0; i < file_num; i++){
		...
		len = (long)strlen(buf + HEADER_EACH);
		memcpy(buf + 40, &len, 2);
	...
	file_ctx_r *files)	// 各ソース・ファイルの情報
{
	...
	for (j = 0; j < file_num; j++){
		...
		len = (long)wcslen(uni_buf);	// ファイル名の文字数
	...
	unsigned long meta[7])	// サイズ、作成日時、更新日時、ボリューム番号、オブジェクト番号
{
	...
	if (packet_count != -1){
		...
		long i = SetFilePointer(hIniBin, ini_off, NULL, FILE_BEGIN);
		if ((i == INVALID_SET_FILE_POINTER) && (GetLastError() != NO_ERROR)){
			delete_ini_file();
			return;
		}
	...
	source_ctx_r *s_blk)	// 各ソース・ブロックの情報
{
	...
	long i, j, k, max, left, first_time = 0;
	...
	unsigned long size)
{
	...
	rv2 = (unsigned long)(offset >> 32);
	rv = (unsigned long)offset;
	...
	slice_ctx *sc)
{
	...
	//printf("read flake[%d] = %d, size = %d, save ok\n", rv, id, flake_size);
	p = sc->flk_buf;
	...
	slice_ctx *sc)			// スライス検査用の情報
{
	...
	long i, j, max, left, num;
	unsigned long err_off, flag;
	...
	for (i = 0; i < max; i++){
		...
		if (flag & 8){	// 分割された断片
			if ((sc->flake_count >= 0) && (s_blk[j].exist == 0)){	// 未発見のブロックなら
				if (flag == 8){	// 後半の断片
					read_flake(hFile, 0, j, (long)file_off, 0, sc);
				} else {	// 前半の断片
					__int64 file_size;
					memcpy(&file_size, meta, 8);
					read_flake(hFile, file_off, j, (long)(file_size - file_off), 1, sc);
				}
			}
		} else {
...
void write_ini_verify(
	long id,		// ソース・ブロックの番号
	long flag,		// 1=エラー訂正が必要, 8=断片の後半, 9=断片の前半
	__int64 off)
{
	...
	slice_ctx *sc)
{
	...
	// スライスが所属する作業ファイルを開く
	get_temp_name(list_buf + files[num].name, temp_path + base_len);
	sc->hFile_tmp = CreateFile(temp_path, GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
