// verify.c
// Copyright : 2015-12-11 Yutaka Sawada
// License : GPL
...
static void set_splited_filename(
	...
	long dir_len)			// フル・パスにおけるディレクトリ部分の長さ
{
	wchar_t *tmp_p;
...
static long check_extra_part(
	wchar_t *find_name,		// 検出されたファイル名
	wchar_t *file_name,		// 本来のファイル名
	long len)				// 本来のファイル名の文字数
{
	long num = (long)wcslen(find_name);
	if (_wcsnicmp(find_name, file_name, len) == 0){	// 末尾に付加されてる
		...
		len -= (long)wcslen(file_ext);	// 本来のファイル名の長さから、拡張子の長さを引く
	...
	file_ctx_r *files)		// 各ソース・ファイルの情報
{
	...
	for (i = 0; i < entity_num; i++){
		...
		if ((files[i].state & 7) == 0)
			continue;

		len = (long)wcslen(list_buf + files[i].name);
...
static long binary_search(
	long *order,		// 配列
	long high,			// 配列の要素数 = 探索範囲の上限要素 +1
	unsigned long key,	// 目的の値
	source_ctx_r *s_blk)
{
	long mid, low = 0;	// 探索範囲の下限要素
...
// Richard Newcombe による改良型のクイックソート
	...
	source_ctx_r *s_blk)
{
	...
	while (right > left){
		...
		part = order[left];
		while (loop1 < loop2){
			while (s_blk[order[loop2]].crc > s_blk[part].crc)
				loop2--;
			order[loop1] = order[loop2];
			while ((loop1 < loop2) && (s_blk[order[loop1]].crc <= s_blk[part].crc))
				loop1++;
			order[loop2] = order[loop1];
		}
		order[loop1] = part;
		quick_sort(order, left, loop1 - 1, s_blk);
		left = loop1 + 1;
	}
}

// 二分探索法を改造して一致しない場合は high 側を返す
static long binary_search_high(
	unsigned long *keys,	// 配列
	long low,				// 探索範囲の下限要素
	long high,				// 配列の要素数、探索範囲の上限要素+1
	unsigned long key)		// 目的の値
{
...
static void init_index_search(
	unsigned long *keys,	// データの配列
	long data_count,		// データ数
	long *index,			// 目次
	long index_bit)			// 目次の大きさ、(32-bit値 >> shift) = 目次の項目
{
	long i, off = 0;
...
// 目次の大きさによって探索速度が変わる。
static long index_search(
	unsigned long *keys,	// 配列
	long data_count,		// 配列の要素数 = 探索範囲の上限要素 +1
	unsigned long key,		// 目的の値
	long *index,			// 目次
	long index_shift)		// (32-bit値 >> shift) = 目次の項目
{
	long i;
	...
	slice_ctx *sc)
{
	...
	long i, j, num, file_block, block_count, index_bit, short_count;
	long *order = NULL, *short_crcs;
	...
	for (num = 0; num < entity_num; num++){
		if ((files[num].size > 0) && ((files[num].state & 0x80) == 0)){	// チェックサムがあるファイルだけ比較する
			file_block = (long)(files[num].size / (__int64)block_size);	// フルサイズのブロックの数
			block_count += file_block;
			last_size = (unsigned long)(files[num].size - ((__int64)file_block * (__int64)block_size));
			if (last_size > 0){
				short_count++;	// 半端なブロックを含む
				if (sc->min_size > last_size)
	...
	i = sizeof(long) * ((block_count * 2) + (1 << index_bit));
	if (short_count > 0)
		i += sizeof(long) * entity_num + entity_num;	// short_crc 用の領域
	order = (long *)malloc(i);
	if (order == NULL)
	...
	for (num = 0; num < entity_num; num++){
		if ((files[num].state & 0x80) == 0){	// チェックサムがあるファイルだけ比較する
			...
			for (i = files[num].b_off; i < files[num].b_off + file_block; i++){
				order[j] = i;	// 最初はブロック番号にしておく
				j++;
			}
		}
	}
	// 昇順に並び替える
	quick_sort(order, 0, block_count - 1, s_blk);
	// 並び替えられた CRC も記録しておく
	...
	// 詳細検査用の作業領域を確保する
	if (((switch_v & 1) == 0) && (block_size * 3 < get_mem_size(0)))
		sc->buf = (unsigned char *)malloc(block_size * 3);
	if (sc->buf == NULL)
		return 1;	// メモリー不足でスライド検査できない、簡易検査にする

//last_size = GetTickCount();
	if (((switch_v & 1) == 0) && (short_count > 0)){	// 半端なブロックを比較する準備
		...
		for (num = 0; num < entity_num; num++){
			...
			if ((files[num].state & 0x80) == 0){	// チェックサムがあるファイルだけ比較する
				if (files[num].size % (__int64)block_size){
					//file_block = (long)(files[num].size / (__int64)block_size);	// フルサイズのブロックの数
					...
					short_use[num] = 1;	// CRC-32 未計算の印
				}
			}
		}
	}
//last_size = GetTickCount() - last_size;
//printf("reverse CRC: %d mil sec\n", last_size);
	...
	sc->h = (HANDLE)_beginthreadex(NULL, 65536, thread_read, (LPVOID)sc, 0, NULL);
	if (sc->h == NULL){
		free(sc->buf);
		sc->buf = NULL;
		CloseHandle(sc->run);
...
void save_flake(
	unsigned char *buf,
	long id,			// ブロック番号
	long flake_size,	// 断片のサイズ
	long side,			// どちら側か 1=front, 0=rear
	slice_ctx *sc)
{
	unsigned char *p;
	long i, rv;
	...
	if (sc->flake_count > 0){	// 保管場所があるか調べる
		...
		for (i = 0; i < sc->flake_count; i++){
			...
			if (fc->id == id){	// 既にスライス断片が記録されてる
				//printf("flake[%d] = %d, size = %d, add ok\n", i, id, flake_size);
				if (side){	// 前半に上書きコピーする
					memcpy(p, buf, flake_size);
					fc->front_size = flake_size;
					if (fc->rear_size < 0)	// 検査済みの印を消す
						fc->rear_size = -(fc->rear_size);
				} else {	// 後半に上書きコピーする
					memcpy(p + (block_size - flake_size), buf, flake_size);
					fc->rear_size = flake_size;
					if (fc->front_size < 0)	// 検査済みの印を消す
						fc->front_size = -(fc->front_size);
				}
				return;	// 追加して終わる
	...
	if (rv == -1){	// スライス断片の記録領域を拡張する
		long max_count;
		__int64 mem_size;
		mem_size = get_mem_size(0) - (4 << 20);	// 4MB 余裕を見ておく
		max_count = (long)(mem_size / (__int64)(sizeof(flake_ctx) + block_size));
		if (max_count > source_num / 2)
			max_count = source_num / 2;	// 原理的に分割断片はソース・ブロック数の半分以下
		//printf("mem_size = %I64d MB, max_count = %d \n", mem_size >> 20, max_count);
		if (sc->flake_count < max_count){
			i = sc->flake_count + 1;
			i *= sizeof(flake_ctx) + block_size;
			p = (unsigned char *)realloc(sc->flk_buf, i);
			if (p != NULL){
				sc->flk_buf = p;
	...
	if (rv < sc->flake_count - 1){	// 末尾領域以外が空いてるなら
		i = (sizeof(flake_ctx) + block_size) * rv;
		p = sc->flk_buf + i;
		i = (sizeof(flake_ctx) + block_size) * (sc->flake_count - rv - 1);
		//printf("slide from %d to %d, %d bytes \n", p + ((sizeof(flake_ctx) + block_size)), p, i);
		memmove(p, p + ((sizeof(flake_ctx) + block_size)), i);
		rv = sc->flake_count - 1;	// 前にずらして末尾領域に保管する
	}

	//printf("flake[%d] = %d, size = %d, save ok\n", rv, id, flake_size);
	p = sc->flk_buf;
	p += rv * (sizeof(flake_ctx) + block_size);
	fc = (flake_ctx *)p;
	p += sizeof(flake_ctx);
	if (side){	// 前半に上書きコピーする
		memcpy(p, buf, flake_size);
...
long check_flake(
	...
	long switch_v,				// +16=検出後に保存
	file_ctx_r *files,			// 各ソース・ファイルの情報
	source_ctx_r *s_blk,		// 各ソース・ブロックの情報
	slice_ctx *sc)
{
	unsigned char *p, err_mag;
	long i, j, rv, num, err_off;
	flake_ctx *fc;
	...
	for (j = 0; j < sc->flake_count; j++){
		...
		if (i != -1){	// スライス断片が記録されてる
			if (s_blk[i].exist != 0){	// ブロックの断片を保管した後に、そのブロックが発見されていたら
			...
			} else if (fc->front_size + fc->rear_size + 1 >= (long)block_size){	// 前半と後半の合計がブロック・サイズ以上
				// 分割を試すスライスはブロック・サイズのものだけ
				// 検査済みだと両方とも負の値なので、合計してもブロック・サイズを超えないはず
				rv = correct_error(p, s_blk[i].size,
							s_blk[i].hash, s_blk[i].crc, &err_off, &err_mag);
				...
				if (rv >= 0){	// 完全なブロックを新たに発見した (発見済みは先に除外してる)
					...
					first_num++;
					fc->id = -1;	// 比較が終わったら領域を空ける
				} else {	// ハッシュ値が異なる場合は、検査済みの印として負にする
					fc->front_size = -(fc->front_size);
					fc->rear_size = -(fc->rear_size);
				}
			}
...
static long search_block_simple(
	wchar_t *temp_path,			// 作業用、基準ディレクトリが入ってる
	long num1,					// file_ctx におけるファイル番号 (未知なら -1)
	unsigned long last_size,	// 末尾ブロックのサイズ
	wchar_t *file_name,			// 表示するファイル名
	HANDLE hFile,				// ファイルのハンドル
	__int64 file_size,			// 存在するファイルのサイズ
	__int64 last_off,			// どこから検査を開始するか (ここまでは既に検査済み)
	long switch_v,				// +16=検出後に保存, +32=後方からは検索しない
	file_ctx_r *files,			// 各ソース・ファイルの情報
	source_ctx_r *s_blk,		// 各ソース・ブロックの情報
	slice_ctx *sc)
{
	unsigned char hash[20];
	long i, j, b_last, find_num, find_flag, find_next, short_next;
	long block_count, num, i1, i2, i3, i4, miss_num, miss_max;
	long *order, *index, index_shift;
	unsigned long crc, *crcs;
	...
	miss_max = (long)((file_size >> 4) / (__int64)block_size);	// size/16 ブロック個まで (6%)
	...
	if (num1 >= 0){	// ファイル番号が指定されてるなら
		...
		find_next = (long)(last_off / (__int64)block_size);	// 検査開始位置のブロック番号
	...
	if (block_count > 0){
		...
		while (file_off + (__int64)block_size <= file_size){
			...
			if ((find_flag < 0) && (file_md5_crc32_block(hFile, file_off, block_size, hash) == 0)){
...
/*				if (find_flag == 0){
			...
			if (GetTickCount() - time_last >= UPDATE_TIME){
				if (print_progress_file((long)((file_off * 1000) / file_size), first_num, file_name))
					return -2;
				time_last = GetTickCount();
			}
		}
	...
	if ((num1 >= 0) && (files[num1].b_num >= 2) && (file_size >= last_size) && ((s_blk[b_last].exist & 0x1000) == 0)){
		...
		} else if (file_size >= block_size + last_size){	// ブロック倍よりも大きいなら
			file_off = last_off;
		} else {	// 末尾ブロックのサイズなら
			file_off = 0;
		}
	...
	if ((switch_v & 32) && (file_size <= files[num1].size / 2))
		return find_num;
	...
	if ((s_blk[b_last].exist & 0x1000) == 0){
		miss_num = 1;
		if ((file_off >= last_off) && (file_md5_crc32_block(hFile, file_off, last_size, hash) == 0)){
			i = b_last;
			if (memcmp(hash, s_blk[i].hash, 20) == 0){	// チェックサムが一致するか確かめる
				if (s_blk[i].exist == 0){	// 未発見のブロックなら
					if (switch_v & 16){	// コピーする
...
/*				if (find_flag == 3){
			...
			// 経過表示
			if (GetTickCount() - time_last >= UPDATE_TIME){
				if (print_progress_file((long)(((file_size - file_off) * 1000) / (file_size - last_off)), first_num, file_name))
					return -2;
				time_last = GetTickCount();
			}
		}
...
#define FAIL_MAX 3
#define FAIL_LIMIT 13

// スライド検査でソース・ブロックを探す (サブ・スレッドがブロックを読み込む)
// ブロック・サイズ * 3 < 4GB であること
// -2=キャンセル, -1=エラー, 0=見つからない, 1〜=見つかった個数
static long search_block_slide(
	wchar_t *temp_path,			// 作業用、基準ディレクトリが入ってる
	long num1,					// file_ctx における初期状態のファイル番号 (未知なら -1)
	unsigned long last_size,	// 末尾ブロックのサイズ
	wchar_t *file_name,			// 表示するファイル名
	HANDLE hFile,				// ファイルのハンドル
	__int64 file_size,			// 存在するファイルのサイズ
	...
	slice_ctx *sc)
{
	unsigned char *buf, hash[16], err_mag, *short_use;
	long i, j, find_num, find_flag, find_next, find_last, short_next;
	long block_count, short_count, num, i1, i2, i3, i4;
	long *order, *index, index_shift;
	unsigned long len, off, end_off, err_off;
	unsigned long prev_crc, fail_count, rear_off;
	unsigned long crc, *crcs, short_crc, *short_crcs;
	unsigned long time_last;
	...
	if (num1 >= 0){	// ファイル番号が指定されてるなら
		...
		if (file_size > files[num1].size){
			rear_off = (unsigned long)((file_size - files[num1].size) % (__int64)block_size);
		} else if (file_size < files[num1].size){
			rear_off = block_size - (unsigned long)((files[num1].size - file_size) % (__int64)block_size);
		}
	...
	short_crcs = index + (long)(1 << (32 - index_shift));
	short_use = (unsigned char *)(short_crcs + entity_num);

	// 最初はブロック・サイズの倍まで読み込む
	if (file_size < file_off + ((__int64)block_size * 2)){
		len = (unsigned long)(file_size - file_off);
		memset(buf + len, 0, (block_size * 2) - len);
	} else {
		len = block_size * 2;
	}
	if (file_read_data(hFile, file_off, buf, len))
		memset(buf, 0, len);	// 読み込み時にエラーが発生した部分は 0 にしておく
	...
	if ((short_count > 0) && (last_off == 0)){
		//printf("tiny search from %I64d, size = %d\n", file_off, len);
		for (num = 0; num < entity_num; num++){	// ファイル番号
			if (short_use[num] == 0)
				continue;	// 半端なブロックを含まないファイルは除外する
			i = files[num].b_off;	// 先頭ブロック = 半端ブロックの番号
			last_size = s_blk[i].size;
			// 未発見で、検査データより小さな、小さなファイルの半端なブロックなら
			if ((last_size < block_size) && (last_size <= len + 1) && ((s_blk[i].exist & 0x1000) == 0)){
				crc = crc_update(0, buf, last_size);
				if (short_use[num] == 1){	// パディング部分を取り除いた CRC-32 を逆算する
					short_crcs[num] = crc_reverse_zero(s_blk[i].crc, block_size - last_size);
					short_use[num] = 2;	// CRC-32 計算済みの印
				}
				short_crc = short_crcs[num];
				//printf("tiny %d, size = %d, CRC = 0x%08X, 0x%08X\n", num, last_size, crc, short_crc);
				if (crc == short_crc){	// CRC-32 が一致した
	...
	if (((block_count > 0) && ((file_off + (__int64)block_size <= file_size)
		...
		if (file_size >= (__int64)block_size){
			...
			end_off = (unsigned long)file_size;
		}
		...
		while (1){
			...
			if (file_next < file_size){	// ファイルの終端に達していなければ、残りを読み込む
				if ((file_next + (__int64)block_size) > file_size){
					len = (unsigned long)(file_size - file_next);
				} else {
					len = block_size;
				}
				sc->size = len;
			...
			while (off < end_off){
				...
				if ((short_next >= 0) && (file_off + off == short_off)){	// 半端なブロックなら
					...
					short_crc = short_crcs[num];
					//printf("short[%d], size = %d, short_crc = 0x%08X\n", i, s_blk[i].size, short_crc);
					find_flag = correct_error(buf + off, s_blk[i].size, s_blk[i].hash, short_crc, &err_off, &err_mag);
					if (find_flag == 0)
						find_flag = 2;
...
//					j = index_search(crcs, block_count, crc, index, index_shift);
					...
					for (; j < block_count; j++){	// 複数ブロックが一致するかもしれないので
						if (crcs[j] != crc)
							break;	// CRC-32 が一致しなくなったら抜ける
						i = order[j];
						// 前回ブロックを検出した範囲では、検出済みのブロックを無視する
						if ((file_off + off < last_off) && ((s_blk[i].exist & 0x1000) != 0))
							continue;
				...
				if (find_flag >= 0){	// その位置でブロックのチェックサムが一致すれば
					...
					if (find_flag == 1)	// 訂正したエラーを戻しておく
						buf[err_off + off] ^= err_mag;
					if (find_flag > 3){	// スライド検査で見つけた場合
						if (file_off + off + 1 < last_off){	// 前回のブロックと今回のが重なってるなら
							find_flag = 5;	// 重なってない所から次のブロックを探す (三つのブロックが重なるのは拒否する)
							err_off = (unsigned long)(last_off - file_off);	// 2バイト以上ずれる
							//printf("cover [%I64d to %I64d], last [%I64d to %I64d], next off = %d\n",
							//		file_off + off, file_off + off + block_size, last_off - block_size, last_off, err_off);
...
/*					if (find_flag == 1){
					...
					if ((sc->flake_count >= 0) &&
							(file_off + (__int64)(off + s_blk[i].size + block_size - 1) > file_size) &&
							(file_off + (__int64)(off + s_blk[i].size) + 1 < file_size)){	// 分割断片は 2〜ブロック・サイズ-2 バイト
						...
						if ((i1 < i3) && ((s_blk[i1].exist & 0x1000) == 0)){	// ファイル内で未発見なら
							i2 = (long)(file_size - file_off) - off - s_blk[i].size;	// 断片のサイズ
							//printf("slice[%3d] : front flake size = %d \n", i1, i2);
							find_num++;
							write_ini_verify(i1, 9, file_off + (__int64)(off + s_blk[i].size));	// 断片の位置
							if (s_blk[i1].exist == 0)	// 未発見なら後半の断片を保管する
					...
					if (find_flag <= 3){	// 見つけたのが順当なブロックなら
						...
						if (crc == prev_crc){	// 同じ CRC のブロックを重なった範囲内で発見しないようにする
							...
							//printf("skip %d after find\n", off - off2);
						}
						if ((find_flag == 5) && (off < err_off)){
							off = err_off;	// 前回発見したブロックの終了位置
							if (off <= end_off)
								crc = crc_update(0, buf + off, block_size);
							//printf("off = %d after cover\n", off);
						}
					}
					fail_count = 0;	// 誤検出回数を初期化する
					fail_off = file_off + off;	// スライド検査の開始位置
				} else {	// 発見できなかったのなら 1バイトずらす
					prev_crc = crc;
					crc = CRC_SLIDE_CHAR(crc, buf[off + block_size], buf[off]);
					off++;
					if (crc == prev_crc){	// 1バイトずらしても同じ CRC なら、CRC が同じ区間は無視する
						// 異なるデータで同じ CRC になるスライス組があっても、
						// スライド中に必ず CRC が変化するので、これのせいで見逃すことは無い！
						//long off2 = off - 1;
						...
						fail_count++;
						//printf("MD5 fail %u, off = %I64d, Ave = %u\n", fail_count, file_off + off,
						//		(unsigned long)((file_off + off - fail_off) / (__int64)fail_count));
						if (fail_count == FAIL_MAX){
							// CRC が同じで MD5 が異なることが多すぎ (直近 3回の発生間隔が平均 8KB 以下) なら
...
/*				if ((file_next + (__int64)block_size) > file_size){
					...
					len = (unsigned long)block_size;
				}
				if (!ReadFile(hFile, buf + block_size, len, &i, NULL) || (len != (unsigned long)i))
					memset(buf + block_size, 0, len);	// 読み込み時にエラーが発生した部分は 0 にしておく
*/
				WaitForSingleObject(sc->end, INFINITE);	// サブ・スレッドの読み込み終了の合図を待つ
				// 読みこみ時にエラーが発生していても続行することに注意！
				...
				end_off = (unsigned long)(file_size - file_off - (__int64)block_size);	// 最後のループだけ検査範囲が狭い
	...
	if ((short_count > 0) && (last_off + (__int64)(sc->min_size) <= file_size)){
		if ((file_off == 0) && (file_size <= (__int64)block_size * 2)){	// 先頭部分は既に読み込んでる
			...
			if (file_off < last_off){
				file_off = last_off;
				len = (long)(file_size - file_off);
			}
			i1 = -1;	// まだファイルからデータを読み込んでない印
		}
		...
		for (num = 0; num < entity_num; num++){	// ファイル番号
			...
			if ((last_size <= len) && ((s_blk[i].exist & 0x1000) == 0)){
				...
				if (crc_update(0, buf + off, last_size) == short_crcs[num]){	// CRC-32 が一致した
					data_md5_block(buf + off, s_blk[i].size, hash);
					if (memcmp(hash, s_blk[i].hash, 16) == 0){	// 更に MD5 も一致すれば
...
static long show_file_slice(
	char *ascii_buf,
	file_ctx_r *files,		// 各ソース・ファイルの情報
	source_ctx_r *s_blk)	// 各ソース・ブロックの情報
{
	long i, id, find_num, find_total;
	...
	HANDLE hFile_tmp)	// 作業ファイルのハンドル
{
	...
	unsigned long time_last, block_end;
	__int64 copy_end;
	...
	while (copy_size > 0){
		...
		if (block_end >= block_size){	// ブロック・サイズごとに
			...
			// 経過表示
			if (GetTickCount() - time_last >= UPDATE_TIME){
				if (print_progress_file((long)((copy_end * 1000) / file_size), -1, file_name))
					return 2;
				time_last = GetTickCount();
			}
		}
	...
	slice_ctx *sc)			// スライス検査用の情報
{
	...
	long i, find_num, b_last, dir_len, name_off, name_len, num1;
	unsigned long last_size, meta_data[7];
	...
	last_size = (unsigned long)(files[num].size % (__int64)block_size);
	if (last_size == 0)
	...
	if (files[num].state & 0x1A){	// 破損 0x02、追加 0x10、破損して別名 0x28 ならソース・ファイルを検査する
		...
		find_num = check_ini_verify(file_name, hFile, num1, meta_data, switch_v, files, s_blk, sc);
		...
		if (file_size + 1 < (__int64)(sc->min_size)){	// 小さすぎるファイルは調べない
			...
			if (files[num].state & 2){	// 破損なら上位 24-bit に先頭の完全なスライス数が記録されてる
				comp_num = (unsigned long)(files[num].state) >> 8;
			...
			if (find_num >= 0){	// 検査結果の記録がある
				...
				if (switch_v & 1){	// 簡易検査
					find_num = search_block_simple(temp_path, num1, last_size, file_name,
							hFile, file_size, file_off, switch_v, files, s_blk, sc);
				} else {	// 詳細検査
					find_num = search_block_slide(temp_path, num1, last_size, file_name,
							hFile, file_size, file_off, switch_v, files, s_blk, sc);
				}
				if (find_num >= 0){	// 検査結果を記録する
					write_ini_verify2(num1, meta_data, switch_v, find_num);
					find_num += comp_num;
				} else {	// エラーが発生したら
					write_ini_verify2(num1, meta_data, switch_v, -1);	// 検査中の記録を破棄する
				}
			}
		}
		...
		if (find_num > 0){	// スライスが検出された時
			if (sc->flake_count > 0){	// スライス断片の一致を調べる
				if (check_flake(temp_path, switch_v, files, s_blk, sc) != 0)
					return 1;
			}
	...
	dir_len = base_len + (long)wcslen(file_path);	// ディレクトリ部分の長さ
	...
	if (hFind == INVALID_HANDLE_VALUE)
		return 0;
	name_off = (long)(offset_file_name(file_name) - list_buf);	// 見つけたファイルとの比較用にファイル名の相対位置
	name_len = (long)wcslen(list_buf + name_off);
	do {
		...
		if (hFile != INVALID_HANDLE_VALUE){	// ファイルを開けたなら内容を検査する
			prog_last = -1;	// 経過表示がまだの印
			find_num = check_ini_verify(FindData.cFileName, hFile, num1, meta_data, switch_v, files, s_blk, sc);
			...
			if (find_num == -3){	// 検査結果の記録が無ければ
				if (switch_v & 1){	// 簡易検査
					find_num = search_block_simple(temp_path, num1, last_size, FindData.cFileName,
								hFile, file_size, 0, switch_v | 32, files, s_blk, sc);
				} else {	// 詳細検査
					find_num = search_block_slide(temp_path, num1, last_size, FindData.cFileName,
								hFile, file_size, 0, switch_v, files, s_blk, sc);
				}
				// 検査結果を記録する
				write_ini_verify2(num1, meta_data, switch_v, find_num);
			}
...
{
	wchar_t temp_path[MAX_LEN], *file_name;
	long find_num, dir_len, name_off, name_len;
	...
	do {
		...
		if (hFile != INVALID_HANDLE_VALUE){	// ファイルを開けたなら内容を検査する
			prog_last = -1;	// 経過表示がまだの印
			find_num = check_ini_verify(FindData.cFileName, hFile, -1, meta_data, switch_v, files, s_blk, sc);
			//printf("ini = %d, file_size = %I64d\n", find_num, file_size);
			if (find_num == -3){	// 検査結果の記録が無ければ
				if (switch_v & 1){	// 簡易検査
				...
				// 検査結果を記録する
				write_ini_verify2(-1, meta_data, switch_v, find_num);
			}
			CloseHandle(hFile);	// 検査したファイルを閉じる
			if (add_file_path(file_path)){	// 検査済みファイルのファイル名を記録する
				printf("add_file_path\n");
...
fclose(fp);
	...
	while (list2_off < list2_len){
		...
		if (hFile != INVALID_HANDLE_VALUE){
			prog_last = -1;	// 経過表示がまだの印
			find_num = check_ini_verify(file_name, hFile, -1, meta_data, switch_v, files, s_blk, sc);
			if (find_num == -3){	// 検査結果の記録が無ければ
				if (switch_v & 1){	// 簡易検査
			...
			if (find_num < 0){
				...
				if (sc->flake_count > 0){	// スライス断片の一致を調べる
					if (check_flake(file_path, switch_v, files, s_blk, sc) != 0)
						return -1;
				}
				print_progress_file(-1, first_num, NULL);	// 重複を除外した利用可能なソース・ブロック数を表示する
...
{
	unsigned char hash[20];
	long i, j, k, num, b_last, lost_count, block_count;
	long zero_count, reve_count, dupl_count;
	long *order = NULL;
	...
	if (block_count > 0){
		// CRC-32 の順序を格納するバッファーを確保する
		order = (long *)malloc(block_count * 4);
		if (order == NULL)
			return 0;
		block_count = 0;
		for (num = 0; num < entity_num; num++){
			if ((files[num].state & 0x80) == 0){	// チェックサムがあるファイルだけ比較する
				...
				for (i = files[num].b_off; i < b_last; i++){
					if (s_blk[i].exist != 0){	// 比較可能なスライス
						order[block_count] = i;	// 最初はブロック番号にしておく
						block_count++;
					}
				}
			}
