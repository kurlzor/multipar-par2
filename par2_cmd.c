// par2_cmd.c
// Copyright : 2016-06-11 Yutaka Sawada
// License : GPL
...
static void print_environment(void)
{
	...
	printf("CPU cache\t: %d KB per set\n", (cpu_flag & 0xFFFF8000) >> 10);
...
static int check_block_size(unsigned int adjust_size, int alloc_method, int limit_num)
{
	...
	unsigned int base_size, max_size, min_size, new_size;
	...
	if (alloc_method > 0){	// ブロック・サイズが指定された
		target = alloc_method;
		if (target >= (int)block_max_size){	// ブロック・サイズの範囲内にする
			target = block_max_size;
			block_size = target;
		} else if (target <= (int)block_min_size){
			target = block_min_size;
			block_size = target;
		} else {	// 単位の倍数で指定されたサイズに近い方にする
			block_size = ((target + (base_size / 2)) / base_size) * base_size;
		}
		source_num = 0;
		for (i = 0; i < file_num; i++){
...
*/
	if (pad_size > 0){	// 近辺で効率が良くなる所を探す
		if (alloc_method == 0){	// ブロック数が指定された
			...
			while ((loop_count < MAX_ADJUST * 2) && (new_size <= block_max_size)){
				loop_count++;
				if ((int)new_size > target + (target / 2))
			...
			while ((loop_count < MAX_ADJUST) && (new_size >= block_min_size)){
				loop_count++;
				if ((int)new_size < target - (target / 4))
...
vl= switch_set & 0x000000C0
lp= switch_set & 0x00000700
rd= switch_set & 0x00003000
ri= switch_set & 0x00004000
*/
	printf("Parchive 2.0 client version %x.%x.%x.%x by Yutaka Sawada\n\n",
		CLIENT_VERSION >> 12, (CLIENT_VERSION >> 8) & 0xF,
		(CLIENT_VERSION >> 4) & 0xF, CLIENT_VERSION & 0xF);
	if (argc < 3){
		printf("Self-Test: ");
		if (par2_checksum(uni_buf) == 0){
			printf("Success");
		} else {
			printf("Incomplete");
		}
	...
	for (i = 2; i < argc; i++){
		...
		if (((tmp_p[0] == '/') || (tmp_p[0] == '-')) && (tmp_p[0] == argv[2][0])){
			...
			// 作成時のオプション (数値)
				...
				switch_b = (switch_b & 0xFFC00003) | k;
				...
				parity_num *= 100;
				if (tmp_p[j] == '.'){	// 小数点があるなら、小数点以下第二位まで読み取る
					j++;
				...
				split_size = 0;	// 分割サイズは 1GB まで
				j = 2;
				while ((j < 2 + 10) && (tmp_p[j] >= '0') && (tmp_p[j] <= '9')){
					split_size = (split_size * 10) + (tmp_p[j] - '0');
					j++;
				}
			// 検査時のオプション (数値)
			...
			// 共通のオプション (数値)
				...
				if ((tmp_p[1] >= '0') && (tmp_p[1] <= '7'))	// 0〜7 の範囲
					memory_use = tmp_p[1] - '0';
		...
		if ((argv[1][0] == 'c') || (argv[1][0] == 't')){	// 作成なら
			...
			if ((j == INVALID_FILE_ATTRIBUTES) || (j & FILE_ATTRIBUTE_DIRECTORY)){
				...
				if (hFind != INVALID_HANDLE_VALUE){
					...
					dir_len = (int)wcslen(search_path);
					do {
						//printf("file name = %S\n", FindData.cFileName);
						name_len = (int)wcslen(FindData.cFileName);
						if ((dir_len + name_len < MAX_LEN) &&	// ファイル名が長すぎない
								(_wcsicmp(FindData.cFileName + (name_len - ext_len), file_ext) == 0)){	// 拡張子が同じ
							file_ext[0] = 0;
...
*/
	...
	if ((argv[1][0] == 'c') || (argv[1][0] == 't')){
		if ((base_dir[0] == 0) && ((switch_set & 0x02) == 0) && (i + 1 < argc)){
			tmp_p = argv[i + 1];
	...
	case 't':
		...
		if (i >= argc){	// もうファイル指定が無いなら
			...
			while (search_path[dir_len] != '\\')
				dir_len--;
			dir_len++;
			if (search_files(search_path, dir_len, 0x20, 0)){	// ファイルだけを探す
				free(list_buf);
				return 1;
			}
...
*/
			...
			for (; i < argc; i++){
				...
				while (search_path[dir_len] != '\\')
					dir_len--;
				dir_len++;
				if (search_files(search_path, dir_len, switch_set & 0x20, j)){
					free(list_buf);
					return 1;
				}
...
fclose(fp);
		...
		if (total_file_size == 0){	// ファイル・サイズが全て 0 だとブロックも無いはず
			...
			if (parity_num < 0){	// 冗長性(%)でパリティ・ブロック数を決める
		...
		if (parity_num > 0){
			...
			i = (switch_set & 0x3000) >> 12;
			if (i == 0){	// 同じ量ずつ割り振る
				if (recovery_limit == 0){	// 制限値が未設定なら
					if (recovery_num == 0){	// ファイル数も未設定なら
						j = 1;
					} else {
						j = recovery_num;
					}
				} else if (recovery_limit >= parity_num){
					recovery_limit = parity_num;
					j = 1;
				} else {	// 制限数からファイル数を計算する
					if (recovery_limit < 0)
						recovery_limit = -recovery_limit;
					j = (parity_num + recovery_limit - 1) / recovery_limit;
				}
				// ファイル数が未設定または超えてるなら、制限数から計算したファイル数にする
				if ((recovery_num <= 0) || (recovery_num > j)){
					recovery_num = j;
				} else if (recovery_num > parity_num){
				...
				if (parity_num < 10){	// 1,2,3,4
					// 必ず全て異なる為にはファイル数は一定以下でないといけない
					j = 0;	// total count
					k = 0;	// count in the file
					while ((j < parity_num) && (parity_num - j > k)){
						k++;
						j += k;
					}
					//printf("recovery_limit = %d, max = %d\n", recovery_limit, k);
					if (recovery_limit > k)
						recovery_limit = k;
				}
				if (recovery_num > 0){
					if (recovery_num > recovery_limit)
						recovery_num = recovery_limit;
				} else {
					// ファイル数が指定されてない場合は、2の乗数で制限無しと同じファイル数になる
					recovery_num = recovery_limit;
				}
				recovery_limit = (1 << recovery_num) - 1;	// 仮想合計数
			} else {
				if (recovery_limit == 0){	// 制限値が未設定ならパリティ・ブロック数にする
					recovery_limit = parity_num;
				} else if (recovery_limit < 0){	// ソース・ファイルの最大ブロック数を制限値にする
					recovery_limit = -recovery_limit;
					if (split_size != 0){	// ソース・ファイルを分割する場合はその分割サイズまで
		...
		if (split_size != 0)	// 分割サイズは指定された時だけ表示する
			printf("Split size\t\t: %u\n", split_size);
		...
		if (argv[1][0] == 'c'){
			i = par2_create(uni_buf, (switch_set & 0x0700) >> 8, (switch_set & 0x7000) >> 12, j);
		} else {
			i = par2_trial(uni_buf, (switch_set & 0x0700) >> 8, (switch_set & 0x7000) >> 12, j);
		}
		break;
	case 'v':
	case 'r':
...
fclose(fp);
		...
		switch_b &= 7;
		if (argv[1][0] == 'v'){
			i = par2_verify(uni_buf, (switch_set & 0xC0) >> 6);
		} else {
			i = par2_repair(uni_buf, (switch_set & 0xC0) >> 6);
		}
		if (list2_buf)
			free(list2_buf);
		break;
