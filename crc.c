// crc.c
// Copyright : 2016-02-17 Yutaka Sawada
// License : GPL
...
unsigned int crc_update(unsigned int crc, unsigned char *buf, unsigned int len)
{
	...
	// special case; shorter than 16 bytes or miss-alignment
	if (((cpu_flag & 8) == 0) || (len < 16) || ((((ULONG_PTR)buf) & 3) != 0))
	...
	temp128 = crc128;
	crc128 = _mm_clmulepi64_si128(crc128, two_k128, 0);
	temp128 = _mm_srli_si128(temp128, 8);
	crc128 = _mm_xor_si128(crc128, temp128);
	// reduce from 96-bit to 64-bit by multiplication with K6
	temp128 = crc128;
	crc128 = _mm_slli_si128(crc128, 4);
	crc128 = _mm_clmulepi64_si128(crc128, two_k128, 0x10);
	crc128 = _mm_xor_si128(crc128, temp128);
	crc128 = _mm_and_si128(crc128, _mm_set_epi32(0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000));
	...
	crc128 = _mm_clmulepi64_si128(crc128, two_k128, 0);
	crc128 = _mm_xor_si128(crc128, temp128);
	crc128 = _mm_and_si128(crc128, _mm_set_epi32(0x00000000, 0x00000000, 0xFFFFFFFF, 0xFFFFFFFF));
	temp128 = crc128;
	crc128 = _mm_clmulepi64_si128(crc128, two_k128, 0x10);
	crc128 = _mm_xor_si128(crc128, temp128);
	crc128 = _mm_xor_si128(crc128, data128);

#ifndef _WIN64	// 32-bit 版だとうまく動く
	crc = _mm_extract_epi32(crc128, 2);
#else	// 64-bit 版の Visual Studio では _mm_extract_epi32 をコンパイルできない
	crc128 = _mm_srli_si128(crc128, 8);
	crc = _mm_cvtsi128_si32(crc128);
#endif
...
static void compute_result_table(unsigned int result, unsigned int result_array[40])
{
	int i;
...
static void fast_compute_crc_table(unsigned int result_array[32])
{
	int i, j, value;
	unsigned int new_crc;

	window_table[0] = 0; // g_CrcTable[0 & 0xff] ^ (0 >> 8) は常に 0
	for (i = 1; i < 256; i++){
...
unsigned int onepass_window_gen(unsigned int window_size, unsigned int window_table[256])
{
	...
	unsigned int i;
	unsigned int masked_result_array[40];
...
void onepass_window_gen(unsigned int window_size)
{
	...
	compute_result_table(result, masked_result_array);
	fast_compute_crc_table(masked_result_array);
}

/*
// 初期値と最終処理の 0xFFFFFFFF を使ってない CRC のスライドには window_mask は必要ない
void onepass_window_gen_short(unsigned int window_size, unsigned int window_table[256])
{
	unsigned int result = 1;
	unsigned int masked_result_array[40];

	result = crc_update_zero(result, window_size);
	compute_result_table(result, masked_result_array);
	fast_compute_crc_table(masked_result_array, window_table);
}
*/
